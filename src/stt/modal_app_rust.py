"""Modal deployment for Kyutai STT using the Rust server (moshi-server).

This uses the production Rust server which can handle 64+ concurrent sessions
per container (vs 1 session with the Python implementation).

Architecture:
  Client --[PCM]--> Python proxy --[Opus]--> Rust moshi-server
                                         <--[Text]--

Deploy with: uvx modal deploy src/stt/modal_app_rust.py
Dev server: uvx modal serve src/stt/modal_app_rust.py
"""
